...
class CreateTable
{
    ...
    {
        $dbObj = new DBConfig();
        $foreginREFERENCES = $dbObj->student_info_table;

        try {
            $dbObj->returnConnection()->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $table_create="CREATE TABLE IF NOT EXISTS $dbObj->student_attendance_table(
				  ...
				  school_id INT,
                  time  VARCHAR(155),
                  FOREIGN KEY (std_id) REFERENCES $foreginREFERENCES(std_id)
                 )";
            $dbObj->returnConnection()->exec($table_create);

            echo "studentAttendance Table created successfully";
        }catch(Exception $e){
            echo $table_create . "<br> studentAttendance " . $e->getMessage();
        }
    }

    function createSmsStatusTable()
    {
        $dbObj = new DBConfig();
        $foreginREFERENCES = $dbObj->student_info_table;
        try {
            $dbObj->returnConnection()->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $table_create="CREATE TABLE IF NOT EXISTS $dbObj->sms_status_table(
				  ...
				  school_id INT,
				  student_id INT,
                  last_update_time  VARCHAR(155),
                  FOREIGN KEY (student_id) REFERENCES $foreginREFERENCES(std_id)
                 )";
            $dbObj->returnConnection()->exec($table_create);
